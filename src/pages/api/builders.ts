...
function getConfiguredBuilders(url: URL): BuilderCfg[] {
  ...
  return parseBuildersParam(envStr);
}
...
async function sumForDate(addr: string, dateStr: string): Promise<number> {
  ...
  if (!buf) return 0;
  try {
    return sumFees(decompressLz4ToText(buf));
  } catch {
    return 0;
  }
}
  ...
  try {
    ...
    if (builders.length === 0) {
      return json({
        total24h: 0,
        perBuilder: [],
        note: 'No builders configured. Pass ?builders=0xaddr:Label,... or set BUILDER_ADDRESSES env var (lowercase addresses).',
      });
    }
    const now = new Date();
    ...
    const perBuilder: { builder: string; label: string; fee24h: number }[] = [];
    ...
    return json({ total24h, perBuilder: top10 }, 200, 'public, s-maxage=60, stale-while-revalidate=300');
  } catch (e: any) {
    return json({ error: 'server_error', detail: String(e?.message || e) }, 500);
  }
};

function json(obj: unknown, status = 200, cache?: string) {
